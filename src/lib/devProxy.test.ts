...
describe('buildApiUrl', () => {
  ...
  it('does not add a version prefix when API proxy is disabled', () => {
    expect(buildApiUrl('http://api.example.com', 'images/generations', null, false)).toBe(
      'http://api.example.com/images/generations',
    )
  })

  it('supports same-origin root API URLs', () => {
    expect(buildApiUrl('/', 'images/generations', null, false)).toBe('/images/generations')
  })
})
